...
const price = (num: string | number, {float = 2, mode = 0, force = false, separate = 3, format = 'string', unit = ''}: priceOptions = {}): number | string | string[] => {
    try {
        ...
        if (typeof mode === 'string') {
            const floatdir = {
                down: 0, // 向下取整
                half_up: 1, // 按 4 舍 5 入取整
                half_even: 2, // 按 4 舍 6 入 5 看单双取整
                up: 3 // 向上取整
            }
 ...
 * @return {string}
 */
const priceUppercase = (price: number | string, {suffix = true} = {}): string => {
    const fraction = ['角', '分']
 ...
 * @returns {number}
 */
const randomNumber = (min: number = 1, max: number = 100): number => {
    if (min > max) [min, max] = [max, min] // 如果最小值大于最大值，两个变量进行互换
 ...
 * @returns {string}
 */
const randomString = (n: number, opt: any): string => {
    const box = {
 ...
 * @returns {string}
 */
const randomInt = (n: number = 6): string => randomString(n, 'number')
